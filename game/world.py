...
#                         e = TownCenter(render_pos, self.resourcemanager)
#                     if e is not None:
...
#                     self.collision_matrix[grid_pos[0]][grid_pos[1]] = 0
#
...
#
#                             # draw workers
#                             wrkr = self.workers[x][y]
#                             if wrkr is not None:
#                                 screen.blit(wrkr.image,
#                                             (render_pos[0] + self.grass_tiles.get_width() / 2 + camera.scroll.x,
#                                              render_pos[1] - (wrkr.image.get_height() - TILE_SIZE) + camera.scroll.y))
#
#
...
#         matrix = [[1 for x in range(self.grid_length_x)] for y in range(self.grid_length_y)]
#         for x in range(self.grid_length_x):
#             for y in range(self.grid_length_y):
#                 if self.world[x][y]['collision']:
#                     # collision matrix needs to be inverted
#                     matrix[y][x] = 0
#         return matrix
#
